...
function createBridgeApi() {
  ...
  return {
    ...
    // ── In-game: FPS benchmark · Session Boost · Runtime Lab · World Pre-Baker ──
    async getFpsBench(id)              { return call("getFpsBench", { id }); },
    async startFpsBench(id)            { return call("startFpsBench", { id }); },
    async cancelFpsBench()            { return call("cancelFpsBench"); },
    async getBoost()                   { return call("getBoost"); },
    async setBoost(enabled, largePages){ return call("setBoost", { enabled: !!enabled, largePages: !!largePages }); },
    async grantLargePages()            { return call("grantLargePages", {}, 180000); },
    async getRuntimeLab(id)            { return call("getRuntimeLab", { id }); },
    async setGc(id, gc)                { return call("setGc", { id, gc }); },
    async installGraal(id)             { return call("installGraal", { id }); },
    async useBundledJava(id)           { return call("useBundledJava", { id }); },
    async getPrebake(id)               { return call("getPrebake", { id }); },
    async startPrebake(id, radius)     { return call("startPrebake", { id, radius }); },
    async cancelPrebake()              { return call("cancelPrebake"); },
